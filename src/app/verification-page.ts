...
@Component({
  ...
  template: `<main>
    <a [href]="pending?.returnPath || '/issues'">← Back to your question</a>
...
export class VerificationPage {
  ...
  constructor() {
    ...
    if (p && !p.verificationOrder) {
      ...
      p.verificationAttemptId = crypto.randomUUID();
      savePending(p);
    }
    this.order = p?.verificationOrder || ['phone', 'payment'];
  ...
  private track(event: string, method?: string): void {
    this.analytics.logEvent(event, {
      ...
      attempt_id: this.pending?.verificationAttemptId,
    });
  }
  async choose(method: 'phone' | 'payment'): Promise<void> {
    ...
    try {
      const response = await firstValueFrom(
        this.api.post<{ checkoutUrl?: string; settled: boolean }>(
          'issuenumber/verification/checkout',
          {
            ...
            actionId: this.pending.verificationAttemptId,
          },
      ...
      if (response.settled) {
        ...
        await this.router.navigateByUrl('/answer', { replaceUrl: true });
        return;
      }
