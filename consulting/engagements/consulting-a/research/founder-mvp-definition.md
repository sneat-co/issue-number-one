...
issues but nominate only one personal #1. Scarce, continuously movable support
votes select one #1 at each organizational scope; that issue automatically
...
- The intended scarcity mechanism is one nominated personal #1 plus scarce vote
  allocation, not a hard cap on recording open issues (E3–E4).
...
- Scope-specific scores are required if the same originating issue competes at
  team, department, and company levels (E5). This is a model implication of the
  confirmed flow, not an independently validated product advantage.
...
- Test whether users understand nomination, scope-specific voting, automatic
