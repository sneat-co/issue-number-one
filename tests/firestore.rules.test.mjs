...
before(async () => {
  ...
  await environment.withSecurityRulesDisabled(async (context) => {
    ...
    await setDoc(doc(db, `${root}/verification/alice`), {
      eligible: true,
      chargeId: 'private-charge-reference',
    });
...
test('anonymous clients cannot read raw public or private projections', async () => {
  ...
  await denied(
    getDoc(doc(db, `${root}/categories/country`)),
    'raw catalog read',
  );
  await denied(
    getDoc(doc(db, `${root}/questions/ireland`)),
    'raw published question read',
  );
  await denied(
    getDoc(doc(db, `${root}/questions/ireland/issues/housing`)),
    'raw issue read',
  );
  await denied(
    getDoc(doc(db, `${root}/questions/ireland/answers/alice`)),
    'answer read',
  );
  await denied(
    getDoc(doc(db, `${root}/verification/alice`)),
    'verification read',
  );
});
...
test('authentication does not expose raw creator, answer, or verification data', async () => {
  ...
  await denied(
    getDoc(doc(own, `${root}/questions/pending-question`)),
    'creator pending question read',
  );
  await denied(
    getDoc(doc(own, `${root}/questions/ireland/answers/alice`)),
    'own answer read',
  );
  await denied(
    getDoc(doc(other, `${root}/questions/ireland/answers/alice`)),
    'other answer read',
  );
  await denied(
    getDoc(doc(own, `${root}/verification/alice`)),
    'own verification read',
  );
  await denied(
    getDoc(doc(other, `${root}/verification/alice`)),
    'other verification read',
  );
});
...
test('anonymous and authenticated clients cannot mutate backend-owned state', async () => {
  ...
  await denied(
    setDoc(doc(anonymous, `${root}/questions/ireland/answers/anonymous`), {
      questionId: 'ireland',
      issueId: 'housing',
    }),
  ...
  await denied(
    setDoc(doc(authenticated, `${root}/questions/ireland/answers/alice`), {
      questionId: 'ireland',
      issueId: 'housing',
    }),
  ...
  await denied(
    updateDoc(doc(authenticated, `${root}/questions/ireland/issues/housing`), {
      supporters: 999,
    }),
    'counter update',
  );
  ...
  await denied(
    updateDoc(doc(authenticated, `${root}/questions/ireland`), {
      status: 'hidden',
      indexable: false,
    }),
  ...
  await denied(
    setDoc(doc(authenticated, `${root}/verification/alice`), {
      eligible: true,
    }),
